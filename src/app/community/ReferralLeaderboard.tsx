...
export default function ReferralLeaderboard() {
  ...
  const getRankIcon = (rank: number) => {
    ...
    return <div className="w-8 h-8 bg-gradient-to-br from-darkblue-500 to-darkblue-600 rounded-full flex items-center justify-center"><span className="text-orange text-sm font-bold">#{rank}</span></div>;
  };

  if (loading) return <div className="text-center py-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange mx-auto mb-4"></div><p className="text-gray-400">Loading leaderboard...</p></div>;

  return (
    <div>
      <div className="text-center mb-12">
        <h2 className="text-3xl font-bold mb-4 bg-gradient-to-r from-white via-orange to-white bg-clip-text text-transparent">🏆 Referral Champions</h2>
        <p className="text-gray-400 max-w-2xl mx-auto">Top referrers helping grow the FreightSignal community.</p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto mb-12">
        {leaderboard.slice(0, 3).map((entry) => (
          <div key={entry.rank} className={`text-center p-6 rounded-2xl border shadow-2xl hover:scale-105 transition-all ${
            entry.rank === 1 ? 'bg-gradient-to-br from-orange/30 to-orange/10 border-orange/50' :
            ...
            <div className="text-2xl font-bold text-orange mb-2">{entry.referralCount}</div>
      ...
      <div className="bg-gradient-to-br from-white/15 to-white/5 border border-white/20 rounded-2xl overflow-hidden shadow-2xl">
        <div className="p-6 bg-gradient-to-r from-orange/20 to-orange/10 border-b border-white/10">
          ...
          <tbody>
            {leaderboard.map((e) => (
              <tr key={e.rank} className="border-b border-white/5 hover:bg-white/10">
                ...
                <td className="p-4 text-center text-2xl font-bold text-orange">{e.referralCount}</td>

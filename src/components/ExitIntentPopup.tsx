...
export default function ExitIntentPopup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { setStatus("success"); setMessage("Perfect! Check your inbox for this week's freight intelligence."); setTimeout(() => setIsVisible(false), 3000); }
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setIsVisible(false)}>
      <div className="bg-darkblue border-2 border-orange/30 rounded-2xl p-8 max-w-lg w-full relative shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-orange/10 rounded-full border-2 border-orange/30 mb-4">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Wait — get this week&apos;s top freight rate moves free</h2>
          <p className="text-gray-300 text-sm">Before you go, grab the supply chain intelligence that could save thousands on your next shipment.</p>
        </div>

        {status !== "success" ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email for instant access" required className="w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-gray-400 text-sm focus:border-orange focus:bg-white/20 transition-all" />
            <button type="submit" disabled={status === "loading"} className="w-full bg-orange text-darkblue px-6 py-3 rounded-lg font-semibold text-sm hover:bg-orange-300 transition-all disabled:opacity-50">
              {status === "loading" ? "Subscribing..." : "Send Me This Week's Rate Intelligence"}
            </button>
